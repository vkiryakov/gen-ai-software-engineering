// Triage agent app — ticket list pane (toolbar, search, filter, tabs, rows)
(function () {
const { PriorityTag, StatusTag, Badge, Avatar, Checkbox, Input, Button, IconButton, Tabs, Spinner } = window.TriageDesignSystem_a9a780;

function TicketRow({ t, selected, active, onSelect, onToggle }) {
  const [hover, setHover] = React.useState(false);
  const { STATUS_TAG, CATEGORY_LABEL, relative } = window.TRIAGE_META;
  ...
  return (
    <div
      ...
      <div style={{ paddingTop: '2px' }} onClick={(e) => { e.stopPropagation(); onToggle(t.id); }}>
...
function FilterPopover({ statusFilter, priorityFilter, categoryFilter, onToggleStatus, onTogglePriority, onToggleCategory, onClear, onClose }) {
  const { STATUSES, STATUS_TAG, PRIORITIES, PRIORITY_LABEL, CATEGORIES, CATEGORY_LABEL } = window.TRIAGE_META;
  ...
  return (
    <div ref={ref} className="triage-filter-popover" style={{
      position: 'absolute', top: '100%', right: 0, marginTop: '6px', width: '240px', maxHeight: '70vh', overflowY: 'auto', zIndex: 20,
      background: 'var(--surface-card)', border: '1px solid var(--border-default)', borderRadius: 'var(--radius-lg)',
      boxShadow: 'var(--shadow-lg)', padding: '14px', display: 'flex', flexDirection: 'column', gap: '14px',
    }}>
      <div>
...
function TicketList({ tickets, loading, activeId, onSelect, tab, onTab, onOpenMenu }) {
  const [selected, setSelected] = React.useState(new Set());
  const [search, setSearch] = React.useState('');
  const [filterOpen, setFilterOpen] = React.useState(false);
  const [statusFilter, setStatusFilter] = React.useState(new Set());
  const [priorityFilter, setPriorityFilter] = React.useState(new Set());
  const [categoryFilter, setCategoryFilter] = React.useState(new Set());
  const searchIcon = <i data-lucide="search" style={{ width: 16, height: 16 }} />;
  const toggle = (id) => setSelected((s) => { const n = new Set(s); n.has(id) ? n.delete(id) : n.add(id); return n; });
  const toggleInSet = (setter) => (v) => setter((s) => { const n = new Set(s); n.has(v) ? n.delete(v) : n.add(v); return n; });
  ...
  return (
    <section className="triage-list-pane" style={{ width: 'var(--list-pane-w)', flex: '0 0 auto', height: '100%', display: 'flex', flexDirection: 'column', borderRight: '1px solid var(--border-subtle)', background: 'var(--surface-card)' }}>
      {/* Toolbar */}
      <div style={{ padding: '10px 14px', borderBottom: '1px solid var(--border-subtle)', display: 'flex', flexDirection: 'column', gap: '10px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          <button className="triage-mobile-only" onClick={onOpenMenu} aria-label="Open menu" style={{
            display: 'none', background: 'none', border: 'none', color: 'var(--text-secondary)', cursor: 'pointer', padding: '4px 4px 4px 0',
          }}>
          ...
          <div style={{ position: 'relative' }}>
            <IconButton icon={<i data-lucide="sliders-horizontal" style={{ width: 16, height: 16 }} />} label="Filter"
              variant={activeFilterCount ? 'primary' : 'secondary'} size="sm" onClick={() => setFilterOpen((v) => !v)} />
            {activeFilterCount > 0 && (
              <span style={{
                position: 'absolute', top: -4, right: -4, minWidth: 15, height: 15, borderRadius: '999px',
                background: 'var(--brand-500)', color: '#fff', fontSize: '10px', fontWeight: 700,
                display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '0 3px',
              }}>{activeFilterCount}</span>
            )}
            {filterOpen && (
              <FilterPopover
                statusFilter={statusFilter} priorityFilter={priorityFilter} categoryFilter={categoryFilter}
                onToggleStatus={toggleInSet(setStatusFilter)} onTogglePriority={toggleInSet(setPriorityFilter)} onToggleCategory={toggleInSet(setCategoryFilter)}
                onClear={() => { setStatusFilter(new Set()); setPriorityFilter(new Set()); setCategoryFilter(new Set()); }}
                onClose={() => setFilterOpen(false)}
              />
            )}
          </div>
        </div>
        <Input iconLeft={searchIcon} placeholder="Search tickets…" size="sm" value={search} onChange={(e) => setSearch(e.target.value)} />
      </div>
      {selected.size > 0 && (
      ...
      <div style={{ flex: 1, overflowY: 'auto' }}>
        {loading ? (
          ...
          <TicketRow key={t.id} t={t} selected={selected.has(t.id)} active={t.id === activeId}
